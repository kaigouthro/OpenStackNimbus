...
export interface Instance extends OpenStackResource {
  ...
  powerState: 'Running' | 'Stopped' | 'Paused' | 'Error' | 'Building' | 'Shutoff'; // Added Shutoff
...
export interface Volume extends OpenStackResource {
  ...
  attachments?: { server_id: string, device: string, attachment_id: string, volume_id: string }[];
...
export interface Image extends OpenStackResource {
  ...
  status: 'active' | 'queued' | 'saving' | 'error' | 'deleted' | 'pending_delete' | 'deactivated';
  created: string; // created_at ISO date string
...
// --- Types for Keystone User Management ---
export interface KeystoneDomain {
    id: string;
    name: string;
...
export interface NovaServerRsp { server: NovaRawServer; }
export interface NovaServersRsp { servers: NovaRawServer[]; }
export interface CinderVolumeRsp { volume: Volume; }
export interface CinderVolumesRsp { volumes: Volume[]; }
...
export interface NeutronNetworkRsp { network: NeutronNetwork; }
export interface NeutronNetworksRsp { networks: NeutronNetwork[]; }
...
export interface NeutronSubnetRsp { subnet: NeutronSubnet; }
export interface NeutronSubnetsRsp { subnets: NeutronSubnet[]; }

export interface NeutronPort extends Port {
    // NeutronPort is essentially our Port type
}
export interface NeutronPortRsp { port: NeutronPort; }
export interface NeutronPortsRsp { ports: NeutronPort[]; }

export interface NeutronRouter extends Router {
    // NeutronRouter is essentially our Router type
}
export interface NeutronRouterRsp { router: NeutronRouter; }
export interface NeutronRoutersRsp { routers: Router[]; }


export interface NeutronFloatingIPRsp { floatingip: FloatingIP; }
export interface NeutronFloatingIPsRsp { floatingips: FloatingIP[]; }
export interface GlanceImageRsp { image: Image; } // Assuming Image type matches Glance response
export interface GlanceImagesRsp { images: Image[]; } // Assuming Image type matches Glance response
export interface NeutronSecurityGroupRsp { security_group: SecurityGroup; }
export interface NeutronSecurityGroupsRsp { security_groups: SecurityGroup[]; }
export interface NeutronSecurityGroupRuleRsp { security_group_rule: SecurityGroupRule; }
export interface NeutronSecurityGroupRulesRsp { security_group_rules: SecurityGroupRule[]; }
export interface NovaFlavorRsp { flavor: Flavor; }
export interface NovaFlavorsRsp { flavors: Flavor[]; }
export interface NovaKeyPairRsp { keypair: KeyPair; } 
export interface NovaKeyPairsRsp { keypairs: { keypair: KeyPair }[]; } 
...
export interface InstanceActionLog {
  ...
  user_id: string;
  result?: 'Success' | 'Error'; // This is a derived field, not from API
}

export interface NovaInstanceActionsRsp {
  instanceActions: InstanceActionLog[];
}

// --- Keystone User Management API Response Types ---
export interface KeystoneUsersRsp { users: KeystoneUser[]; }
export interface KeystoneUserRsp { user: KeystoneUser; }
export interface KeystoneDomainsRsp { domains: KeystoneDomain[]; }
