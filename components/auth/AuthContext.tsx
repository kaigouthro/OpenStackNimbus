...
import { ServiceCatalogEntry, KeystoneProject, KeystoneUser, Role } from '../../types';
...
const NIMBUS_IS_ADMIN_KEY = 'nimbus-isAdmin';
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const storedIsAdmin = localStorage.getItem(NIMBUS_IS_ADMIN_KEY);
    ...
    if (storedToken && storedServiceCatalog && storedKeystoneUrl) {
      try {
        ...
        localStorage.removeItem(NIMBUS_IS_ADMIN_KEY);
      }
    }
  }, []);
  ...
  const login = (
    ...
    const userIsAdmin = !!roles?.some(role => role.name.toLowerCase() === 'admin');
    ...
    localStorage.setItem(NIMBUS_IS_ADMIN_KEY, userIsAdmin.toString());

    console.log("AuthContext: Logged in. User:", user?.name, "Project:", project?.name, "Is Admin:", userIsAdmin);
  };
